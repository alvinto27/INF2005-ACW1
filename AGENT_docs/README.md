...
- [Streaming Carrier Plan](STREAMING-CARRIER-PLAN.md#streaming-carrier-plan) — **library refactor implemented; web follow-up open.** The carrier backend interface, two-pass encoding, targeted-read verification, the streamed WAV backend, chunk-size measurements, the memory claim, why `MAX_WAV_FRAME_BYTES` stays for the whole-file helpers, known limits, and the Flask follow-up.
