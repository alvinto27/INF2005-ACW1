...
- [Masked Media Integrity Design](INTEGRITY-DESIGN.md) — the current stage 4c encrypted packet, hashing, signing, receiver bootstrap, verdicts, and limitations, followed by the version 1 design history.
- [Payload Envelope Design](PAYLOAD-ENVELOPE-DESIGN.md) — the caller-added optional seal and content-type header inside `user_payload`, plus their cross-check, handlers, capacity, and limitations; the library encrypts the complete record.
- [Location Confidentiality Plan](LOCATION-CONFIDENTIALITY-PLAN.md) — protocol version 2 design and stage 4c outcome. Holds the field-binding audit, the decode-order invariant, the accepted trade-offs, and the remaining staging plan.
