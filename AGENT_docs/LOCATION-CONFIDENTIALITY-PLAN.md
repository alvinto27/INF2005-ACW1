...
| `MAX_WAV_FRAME_BYTES` = 64 MiB | **deleted.** The whole-file WAV helpers and their allocation cap have been removed. File-backed WAV access uses `WavCarrier`; see [Streaming Carrier Plan, section 10](STREAMING-CARRIER-PLAN.md#10-whole-file-wav-cap) |
...
| 14 | `MAX_WAV_FRAME_BYTES` | **deleted** | the whole-file WAV helpers and their allocation cap were removed; see [Streaming Carrier Plan, section 10](STREAMING-CARRIER-PLAN.md#10-whole-file-wav-cap) |
...
**Status: closed; constant and whole-file helpers deleted.** The file-backed `WavCarrier` provides seekable chunked access, and `read_pcm_wav_info` validates WAV headers without loading the complete file. The [Streaming Carrier Plan](STREAMING-CARRIER-PLAN.md#10-whole-file-wav-cap) is the current record; the analysis above is kept as history.
