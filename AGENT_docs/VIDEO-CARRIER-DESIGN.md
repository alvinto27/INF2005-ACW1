...
One video frame is not one chunk. Decode one frame, then yield bounded RGB slices (target about 1 MiB). At the configured per-frame pixel limit, a 4K RGB frame is about 24 MiB per copy and an 8K frame is about 95 MiB per copy. For normal decoder frame sizes, memory does not grow in proportion to total duration; the 640×360 video-plus-audio RSS test measured 182,648 KiB at 5 seconds and 219,328 KiB at 15 seconds (about 36 MiB more). This is not an absolute hostile-media memory guarantee: audio sample rate, samples per decoded audio frame, and compressed packet size have no explicit caps before FFmpeg/PyAV allocates decoded data. Put the frame's 8-byte timestamp with its **first** slice; later slices have empty fixed bytes. The first audio chunk carries the 8-byte audio-start tick plus its sample high bytes; later audio chunks carry only high bytes.
...
Frame count and audio sample count come from decoding, not container duration or declared counts. PASS 0 below counts and validates them. It adds each decoded frame's RGB units and each audio frame's interleaved sample units to a running total, and stops as soon as the decoded-size limit is exceeded. It also checks the per-frame pixel limit on the stream header before decoding and on every decoded frame; verify runs the same constructor scan.
