...
- Transparent RGBA pixel colour can change even when alpha is zero. The encoder refuses an RGB PNG with a `tRNS` colour key, because embedding can move pixels onto or off the key colour; convert such an image to RGBA first. PNG ancillary data, including text, `eXIf`, `iCCP`, and `tRNS`, and WAV data outside declared PCM samples, such as `LIST` chunks, are outside the hash. Verification does not read them, so a `tRNS` chunk added after encoding does not change the verdict. The output keeps this metadata, but anyone can change it and the verdict stays `Authentic`. Do not use it as evidence.
