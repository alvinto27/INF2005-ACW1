...
| Limit | Detail |
| --- | --- |
| Web request bound | Flask accepts requests up to 256 MiB. Carrier uploads are saved to temporary files, and encoded carriers are written to persistent output files instead of being returned as base64. PNG still requires Pillow to decode the full image, so one decoded image remains resident. Rewriting adds one output image; loading copies public row bands into one backing buffer. Pillow rejects images above twice its configured pixel limit with a clear size error; see [the PNG input errors](PROTOCOL-V3-FULL-MEDIA-HASH.md#4-png-inputs-and-media-contexts). Measured large-image RSS is in [Payload Handling](PAYLOAD-HANDLING.md#png-carrier-memory). See [section 12](#12-web-boundary-follow-up). |
