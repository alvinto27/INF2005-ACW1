...
| FR9 hash verification | Implemented with input requirement | The verifier needs the original cover upload because embedding changes the stego bytes. |
...
| FR11 positive/negative cases | Partially evidenced | Protocol and route tests cover valid PNG, wrong location, tampering, and valid WAV; add captured demo evidence for both media types. |
...
9. `/decode` derives the same location, extracts the packet, verifies the signature, and optionally checks the original-cover hash.
